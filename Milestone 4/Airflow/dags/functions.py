...
def feature_engineering(df):
    ...
    df.to_csv('/opt/airflow/data/fintech_After_FeatureEng.csv',index=False)
    return df
...
def clean_df(filename):
    ...
    lookup_table.to_csv("/opt/airflow/data/look_up.csv",index=False)
    print('Cleaned succesfully')

def bar_no_of_loans_per_state(df):
    ...
    fig = px.bar(
        ...
        x=no_states.index,
        y='Count',
        color=no_states.index,
        color_continuous_scale='Blues',
        title='Number of issued Loan / State'
    )
...
def bar_common_purposes(df):
    ...
    fig = px.bar(
        ...
        color=Purpose.index,
        color_continuous_scale='Blues',
        title='Most Common Purposes for Loans'
    )
    # Customize the layout
    fig.update_layout(
        xaxis_title='Purpose',
    ...
    return fig
def loan_status_per_grade(filtered_df,grade):   
    ...
    grade_distribution.rename(columns={"index": "Loan Status"}, inplace=True)
    grade_distribution["Percentage"] *= 100
    
    fig = px.bar(
        grade_distribution,
        x="Loan Status",
        y="Percentage",
        title=f"Percentage Distribution of Loan Status for Letter Grade: {grade}",
        labels={"Percentage": "Percentage (%)", "Loan Status": "Loan Status"}
    )
    return fig
    
def Home_Mortgage_per_grade(filtered_df,grade):
    # Prepare Home Ownership Pie Chart
    color_map = px.colors.qualitative.Set1  # Predefined color palette
    category_color_map = {category: color_map[i] for i, category in enumerate(filtered_df["Home Ownership"].unique())}
    type_counts = filtered_df['Home Ownership'].value_counts()
    ...
    colors = [category_color_map[label] for label in type_counts.index]
    
    fig = px.pie(
        values=type_counts.values,
        names=labels,
        title=f"Distribution of Home Ownership for Grade {grade} (# of Loans = {filtered_df.shape[0]})",
...
def Hist_Term(df):
    ...
    for term in sorted(df["Term"].unique()):
        df_filtered = df[df["Term"] == term]
        df_filtered = df_filtered.sort_values("Letter Grade")

        # Create the histogram with percentages
        fig = px.histogram(
            df_filtered,
            x="Letter Grade", 
            title=f"Distribution of Letter Grade for Term = {term} (# of Loans = {df_filtered.shape[0]})",
            labels={"Letter Grade": "Letter Grade"},
            histnorm='percent',  # Normalize counts to percentages
